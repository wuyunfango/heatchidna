...
```{r}
samples = dplyr::select(recent_hotspots_au, `datetime`, `longitude`, `latitude`, `power`)
p = ggplot(samples) +
  geom_sf(data = au_map) +
...
vic_map %>% 
  ggplot()+
  geom_sf() +
# geom_point(data=ignitions, aes(x = lon, y = lat)) +
  #geom_density_2d_filled(data=ignitions,aes(x=lon, #y=lat,fill #= ..level..))
  geom_density_2d_filled(data=ignitions,aes(x=lon,y=lat),alpha = 0.5) 
```
