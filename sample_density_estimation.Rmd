...
```{r}
# create dataframe using polygon for plotting
...
r = raster(estimated,crs=crs(au_map))
estimated_df= as.data.frame(r,xy=T,crs=crs(au_map))
...
library(viridis)
ggplot() +
  geom_raster(data=estimated_df, aes(x=x,y=y, fill=layer))+
  coord_quickmap() +
...
```{r}
estimated_df %>% 
  rename(.,long=x,lat=y,prob=layer)-> estimated_df
```


```{r}
modelling <- lightning_ignitions %>% 
  left_join(estimated_df, by=c("long","lat"))
```
