...
sample_hotspots <-read_csv(here::here("model_df.csv"))
set.seed(456)
RFM_small = randomForest(density_result~.,ntree =150, data = sample_hotspots,importance=TRUE)
```
now change the default parameter, fit 1000 trees

```{r}
set.seed(678)
RFM2 = randomForest(density_result~., ntree =1000, mtry = 10, importance = TRUE,data = sample_hotspots)
...
```
```{r}
varImpPlot(RFM_small, scale=TRUE,n.var=20,main="Variable Importance Plot")
```
